...
# Resolve project path to load configuration
PROJECT_ROOT = r"c:\Users\KIIT0001\Documents\GitHub\kai"
...
print("==============================================================")
print("          KAI DEEP-DIVE SECURITY AUDIT & STRESS TEST          ")
print("==============================================================")
...
# ----------------------------------------------------------------
print("--- PART 1: HTTP API Security Controls ---")
...
print("[INFO] Clearing any active process on Port 8000 to ensure we test the newly updated, hardened server...")
try:
...
print("[INFO] Starting a fresh instance of the hardened HTTP API Server...")
...
def run_http_tests():
    ...
    try:
        ...
        print(f"[TEST HTTP-1] GET without credentials: Status Code = {r.status_code}")
        if r.status_code == 401:
            print("  • SUCCESS: Anonymous read request blocked with 401.")
        else:
            print(f"  • FAILURE: Anonymous read request bypass allowed (Status {r.status_code})!")
    except Exception as e:
        print(f"  • ERROR: {e}")
    ...
    for idx, key in enumerate(bad_keys, 1):
        try:
            ...
            print(f"[TEST HTTP-2.{idx}] GET with malformed key '{key}': Status Code = {r.status_code}")
            if r.status_code == 401:
                print("    • SUCCESS: Malformed key blocked.")
            else:
                print("    • FAILURE: Malformed key bypassed authorization!")
        except Exception as e:
            print(f"    • ERROR: {e}")
    ...
    try:
        ...
        print(f"[TEST HTTP-3] POST config without credentials: Status Code = {r.status_code}")
        if r.status_code == 401:
            print("  • SUCCESS: Anonymous write request blocked.")
        else:
            print(f"  • FAILURE: Anonymous config overwrite allowed (Status {r.status_code})!")
    except Exception as e:
        print(f"  • ERROR: {e}")
    ...
    try:
        ...
        print(f"[TEST HTTP-4] GET with valid X-API-Key ('{api_key}'): Status Code = {r.status_code}")
        if r.status_code == 200:
            print("  • SUCCESS: Authorized client authenticated successfully.")
        else:
            print(f"  • FAILURE: Valid API Key was rejected (Status {r.status_code})!")
    except Exception as e:
        print(f"  • ERROR: {e}")
    ...
    try:
        ...
        print(f"[TEST HTTP-5] GET with valid Bearer Token ('{api_key}'): Status Code = {r.status_code}")
        if r.status_code == 200:
            print("  • SUCCESS: Bearer Token authentication successfully validated.")
        else:
            print(f"  • FAILURE: Bearer Token was rejected (Status {r.status_code})!")
    except Exception as e:
        print(f"  • ERROR: {e}")
    ...
    try:
        ...
        print(f"[TEST HTTP-6] GET config details: Status Code = {r.status_code}")
        if r.status_code == 200:
            config_data = r.json()
            ...
            for sk in sensitive_keys:
                if sk in config_data:
                    leak_detected = True
                    print(f"    • FAILURE: Leak detected! Key '{sk}' was exposed in the response payload!")
            
            if not leak_detected:
                print("  • SUCCESS: Config response masked successfully. No credentials or server properties were leaked.")
                print(f"    -> Exposed client-safe fields: {list(config_data.keys())}")
        else:
            print(f"  • FAILURE: Authorized config read failed (Status {r.status_code})!")
    except Exception as e:
        print(f"  • ERROR: {e}")
...
# ----------------------------------------------------------------
print("--- PART 2: MQTT Broker Password Gatekeeper ---")
...
def test_mqtt_auth(user, password, desc):
    ...
    if connected[0]:
        print(f"[TEST AUTH] Connect as '{user}' / '{password}': ALLOWED.")
        return True
    else:
        print(f"[TEST AUTH] Connect as '{user}' / '{password}': REJECTED (Reason/Code: {result_str}).")
        return False
...
for user, pw in bad_credentials:
    ...
    if res:
        ...
        print(f"  • FAILURE: Connection with {desc} was incorrectly allowed!")

if all_bad_blocked:
    print("  • SUCCESS: MQTT Broker authentication is completely secure. Only authorized users can connect.")
else:
    print("  • FAILURE: One or more unauthorized access routes were left open!")
print()
...
# ----------------------------------------------------------------
print("--- PART 3: Granular ACL Topic Privilege Isolation ---")
...
print("[ACL TEST 1] Dashboard subscribing to 'kai/power' (Authorized Read)...")
client_dash_sub.subscribe("kai/power")
time.sleep(0.2)

print("[ACL TEST 2] Dashboard attempting write to 'kai/power' (UNAUTHORIZED Write)...")
...
print("[ACL TEST 3] Control Server publishing to 'kai/power' (Authorized Write)...")
...
if unauthorized_write_blocked and authorized_read_worked:
    print("  • SUCCESS: Dashboard unauthorized write to 'kai/power' was BLOCKED, but authorized read WORKED.")
else:
    if not unauthorized_write_blocked:
        print("  • FAILURE: Dashboard bypassed ACL and successfully published write to 'kai/power'!")
    if not authorized_read_worked:
        print("  • WARNING: Dashboard did not receive valid state on 'kai/power'. Is broker running normally?")
...
print("[ACL TEST 4] Dashboard subscribing to 'kai/control' (UNAUTHORIZED Read)...")
client_dash_sub.subscribe("kai/control")
time.sleep(0.2)

print("[ACL TEST 5] Control Server subscribing to 'kai/control' (Authorized Read)...")
client_ctrl.subscribe("kai/control")
time.sleep(0.2)

print("[ACL TEST 6] Dashboard publishing command to 'kai/control' (Authorized Write)...")
...
if unauthorized_read_blocked and authorized_write_worked:
    print("  • SUCCESS: Dashboard unauthorized read from 'kai/control' was BLOCKED, but authorized write WORKED.")
else:
    if not unauthorized_read_blocked:
        print("  • FAILURE: Dashboard bypassed ACL and successfully read its own/other control command!")
    if not authorized_write_worked:
        print("  • WARNING: Control Server did not receive the dashboard command. Check broker logs.")
...
print("[ACL TEST 7] Dashboard subscribing to wildcard 'kai/#' (UNAUTHORIZED Wildcard Read)...")
...
if wildcard_secured:
    print("  • SUCCESS: Wildcard subscription respected ACL limits. Unauthorized topics were filtered out.")
else:
    print("  • FAILURE: Dashboard successfully sniffed unauthorized 'kai/control' traffic via wildcard subscription!")
...
# ----------------------------------------------------------------
print("--- PART 4: Configuration Injection & Parameter Abuse Mitigation ---")
url_config = "http://127.0.0.1:8000/api/config"
...
try:
    ...
    print(f"[TEST ABUSE-1] Overwrite sensitive settings: Status Code = {r.status_code}")
    if r.status_code == 400:
        print("  • SUCCESS: Attempt to overwrite sensitive settings was strictly BLOCKED by whitelist.")
        try:
            print(f"    → Error details: {r.json().get('error')}")
        except: pass
    else:
        print("  • FAILURE: Protected settings successfully bypassed whitelist validation!")
except Exception as e:
    print(f"  • ERROR: {e}")
...
try:
    ...
    print(f"[TEST ABUSE-2] Inject invalid type: Status Code = {r.status_code}")
    if r.status_code == 400:
        print("  • SUCCESS: Attempt to inject invalid data type was strictly BLOCKED.")
        try:
            print(f"    → Error details: {r.json().get('error')}")
        except: pass
    else:
        print("  • FAILURE: Invalid data type bypassed type verification!")
except Exception as e:
    print(f"  • ERROR: {e}")
...
try:
    ...
    print(f"[TEST ABUSE-3] Inject out-of-bounds number: Status Code = {r.status_code}")
    if r.status_code == 400:
        print("  • SUCCESS: Attempt to inject negative interval was strictly BLOCKED.")
        try:
            print(f"    → Error details: {r.json().get('error')}")
        except: pass
    else:
        print("  • FAILURE: Negative range bypassed bounds verification!")
except Exception as e:
    print(f"  • ERROR: {e}")
...
try:
    ...
    print(f"[TEST ABUSE-4] Submit safe whitelisted preferences: Status Code = {r.status_code}")
    if r.status_code == 200:
        print("  • SUCCESS: Safe preferences successfully updated and merged.")
    else:
        print("  • FAILURE: Valid preferences were rejected!")
except Exception as e:
    print(f"  • ERROR: {e}")
...
if api_process:
    print("[INFO] Terminating temporary HTTP API background process...")
    api_process.terminate()
    api_process.wait()

# Relaunch the live, hardened HTTP API Server for normal system operation
print("[INFO] Relaunching the live, hardened HTTP API Server in the background...")
subprocess.Popen(
    [sys.executable, api_script],
    stdout=subprocess.DEVNULL,
    stderr=subprocess.DEVNULL,
...
print("[OK] Hardened HTTP API is running and live.")
